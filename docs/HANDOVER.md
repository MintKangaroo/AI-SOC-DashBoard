...
인시던트 흐름과 감사·권한·CSP 는 갖춰졌다. 아래 다섯 개는 그 위에 남은 공백이다.
**중요도 순이고, 어느 것도 코드가 없어서 막힌 것은 아니다.**

| # | 비어 있는 칸 | 현재 사실 | 막고 있는 것 |
|---|---|---|---|
| 1 | **Claude AI 분석이 데모** | 운영 `.env` 의 `ANTHROPIC_API_KEY` 가 비어 규칙 기반 폴백으로 동작, 누적 분석 0건. 일일 리포트도 "AI 데모(규칙기반)" | API 키 입력만 |
| 2 | **탐지 성능 측정 불가** | 개별 라벨 0건이라 precision/recall 없음. ML 은 참고용(advisory)이고 탐지 경로 미연결 | 사람의 건별 판정 100건 |
| 3 | **Suricata·Zeek 대기** | 모듈 헬스 `off`, eve.json·notice.log 를 `waiting` 으로 기다리는 중 | `sudo bash scripts/install_ids.sh` |
| 4 | **웹 서버가 개발용** | Werkzeug 로 서비스 중(기동 시 경고). 포트폴리오·데모엔 무방, '운영'이라 부르기엔 미흡 | gunicorn/eventlet 전환 결정 |
| 5 | **데이터의 99.5% 가 합성** | 실측 알림 506건 / 전체 111,348건. 화면의 큰 수치는 대부분 데모 생성기 산출 | 실센서 확대 + 시간 |

※ 오늘 사이드바 사건이 남긴 교훈: **테스트 936건이 전부 초록인 상태에서도 폰으로
   직접 열어본 사람만 찾을 수 있는 결함이 있었다.** 그 종류가 더 있다고 가정할 것.
