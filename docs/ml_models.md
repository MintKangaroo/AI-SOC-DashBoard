...
## 4.5 실트래픽 재학습 경로 (2026-09-08)

실캡처가 개통돼 3초마다 `origin=real` 피처가 쌓이기 시작했습니다(eth0, 약 300~400
pps). 3,000건이 차면 아래 경로로 IF 를 실트래픽으로 다시 학습합니다.
...
- **pps·bps 가 둘 다 0 인 창은 뺀다.** 캡처가 죽어 있던 구간이 정상 프로파일이 되면
  살아 있는 트래픽 전부가 이상으로 보인다. 실제로 표준입력 캡처 사고로 그런 행이
  39건 쌓였었다(삭제·백업함).
- **시간순 뒤 20% 홀드아웃**으로 앞 80% 학습 모델의 이상률을 잰다. 라벨이 없으니
  정확도가 아니라 *분포 이동의 냄새*다 — 오염률 가정의 3배를 넘으면
  `distribution_shift_suspected` 로 표시한다. 그 뒤 전체로 다시 학습해 저장.
- 메타데이터 `data/models/iso_forest_real.json`: 표본 수·구간·오염률·홀드아웃 이상률·
  sklearn 버전·강제 여부. **이 숫자만이 이 모델에 대해 주장할 수 있는 전부**이며
  `eval_ml.py` 의 "1.5 운영 모델" 절이 이것을 읽는다.

**오염률은 측정값이 아니라 가정이다.** 비지도 학습이라 실트래픽의 몇 %가 이상인지
알 수 없고, 이 값이 곧 이상 판정 비율이 된다. 0.05 로 두었다(합성 부트스트랩의 0.08
은 실트래픽에서 하루 2시간을 '이상'으로 만든다). 라벨이 생기면 다시 정한다.

재학습해도 판정은 여전히 참고용(`advisory_only`)이다. 탐지·차단 경로 연결은 사람
라벨로 precision/recall 을 잰 뒤의 일이다(5절).

### 4.6 첫 실트래픽 재학습 실측 (2026-09-08 15:23, 자동)

| 항목 | 값 |
|---|---|
| 표본 | 실트래픽 3,109건 (12:40 ~ 15:23, eth0 약 300~400 pps) · 캡처 공백 1건 제외 |
| 오염률 | 0.05 (가정) → 학습 이상률 5.02% |
| 홀드아웃(뒤 20%) 이상률 | 0.0% — 분포 이동 의심 없음 |
| **합성 부트스트랩 모델의 실트래픽 판정** | 분석 4,476건 중 이상 **2,161건 (48%)** |
| **실트래픽 모델의 판정(교체 후 100건)** | 이상 **0건** |

합성 200샘플로 만든 모델은 실트래픽의 절반을 이상으로 봤다 — 정상 프로파일 구간
자체가 실측과 달랐기 때문이다. 이 숫자가 "합성 모델의 출력은 정상 프로파일 근사치
이상의 의미가 없다"는 문장의 실측 근거다. 실모델 교체 후 0% 는 좋은 신호이지만
**정확도가 아니다**: 라벨이 없으니 놓친 이상이 있는지는 알 수 없고, 홀드아웃 0.0%
는 그 구간이 학습 구간과 같은 종류의 조용한 트래픽이었다는 뜻일 뿐이다. 판정은
여전히 참고용이다.

이 비교가 누적 카운터에 묻히지 않도록 `since_model`(현재 모델의 분석·이상 건수)을
따로 세고 ML 패널에 보여준다.
