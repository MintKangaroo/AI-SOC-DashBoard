...
flowchart TB
  ...
  subgraph Services["서비스 레이어 (51 모듈, 독립 데몬 스레드)"]
    COLLECT["수집: packet_analyzer · sysmon_parser · access_log_parser<br/>authlog_parser · net_monitor · syslog_receiver<br/>snort_monitor · suricata_monitor · zeek_monitor"]
...
자체 호스팅하고 CSP 를 `'self'` 로 좁혔다. JS 23개 파일은 각각 IIFE 로 감싸
