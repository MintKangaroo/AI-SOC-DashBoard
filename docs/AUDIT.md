...
**수정 방향**: 변경된 인시던트 id만 dirty set으로 추적해 그것만 upsert. 락은 dirty set 스냅샷을 뜨는 동안만 잡고 DB I/O는 락 밖에서. **작업량 M**
...
**수정 방향**: B-1의 dirty-set 수정을 하면 이 쿼리 자체가 사라진다. 두 항목은 같은 커밋으로 처리하는 것이 맞다. **작업량 S (B-1에 포함)**
