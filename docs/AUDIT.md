...
| 23 | ~~P3~~ **수정됨** | JS 전역 선언 323개 · 로드 순서 의존 → IIFE 전환으로 전역 **137개**(59% 감소). ES 모듈 전환은 감사 판정대로 하지 않음 | `static/js/dash/*` | ✅ |
...
### E-2. [~~P3~~ **수정됨**] 전역 네임스페이스 오염 · 로드 순서 의존
...
**수정 (2026-08-28)** — 감사 판정대로 **ES 모듈 전환은 하지 않았다.** 권고된 IIFE
대안을 18개 파일 전부에 적용했다. 최상위 선언 335개 중 **공개 137개**만
`Object.assign(window, {...})` 로 명시 노출 — 나머지 198개는 파일 밖에서 보이지
않는다(**59% 감소**).

`window.SOC = {}` 네임스페이스 대신 `window` 직접 노출을 택했다. 인라인 핸들러가
132개 있어 네임스페이스를 도입하면 `onclick="SOC.foo()"` 로 템플릿까지 전부
고쳐야 하는데, 그건 감사가 피하라고 한 대공사와 같은 크기다. IIFE 만으로도
'같은 이름 두 파일 선언 → 로드 시 SyntaxError' 위험은 사라진다.

**착수 전에 세 가지를 먼저 측정했다** — 이게 이 작업의 핵심이었다:

1. **현재 충돌 0건** 확인(선언 335개 전수). 인라인 핸들러 64개도 전부 해소됨.
   즉 오늘 버그는 없고 위험은 잠재적이다.
2. **가변 교차파일 전역**을 찾았다. 값 복사로 내보내면 재대입 후 낡은 참조가
   남는다. 실제 해당은 `alertsDataTable` 하나뿐이어서, 전역 대신
   `redrawAlertsTable()` 함수를 공개하도록 바꿔 없앴다.
   (`globe` 도 후보로 잡혔으나 `fa-globe`·`attack-globe` 오탐이었다.)
3. **JS 가 생성하는 마크업의 `onclick`** — 이걸 빼먹으면 같은 파일에 정의된
   함수가 '내부 전용'으로 오분류되어 감싼 순간 클릭이 조용히 죽는다.
   해당 함수가 **16개** 있었다(`showTechniqueDetail`·`edrKill`·`soarUnblock` 등).
   공개 표면을 (다른 파일 참조) ∪ (템플릿 핸들러) ∪ (생성 마크업 핸들러)
   세 출처로 계산해야 정확하다.

동적 접근(`window[...]`)과 `eval` 은 0건이라 정적 분석으로 충분했다.

**검증**: `tests/js/load_dashboard.js` 가 18개를 실제 로드 순서대로 평가한다
(브라우저 API 는 무엇을 물어도 자기 자신을 돌려주는 Proxy 로 흉내). 전환 전
기준선을 잡고, 전환 후 로드 실패 0 · 필요한 전역 137개 중 누락 0 을 확인했다.
회귀 테스트 4건(`tests/test_frontend_modules.py`): 이름 충돌 0, 전 파일 IIFE,
공개 표면이 전체의 60% 미만, 그리고 **핸들러가 필요로 하는 이름의 실제 공개
여부**. 마지막 것이 이 전환의 유일한 실패 모드다. 변이 주입(공개 누락 / IIFE
해제 / 충돌 주입) 모두 실패 확인.

**남은 위험**: 정적 분석이 닿지 않는 곳은 없으나, 이 하네스는 *로드 시점* 만
검증한다 — 함수 본문의 런타임 오류는 브라우저에서만 드러난다. 그건 이 작업
이전에도 마찬가지였다.
...
#### 1. 로그 스키마 정규화 (OCSF 또는 ECS) — **비추천** · 대안(내보내기 계층) ~~권장~~ **구현됨 (2026-08-29)**
...
**구현 (2026-08-29)** — `modules/ocsf_export.py` + `GET /api/alerts/history/export.ocsf.json`.
권고대로 **전면 도입은 하지 않았다.** 내부 스키마는 그대로 두고 밖으로 나갈 때만
OCSF 1.1.0 Detection Finding(class_uid 2004)으로 바꾼다. 검색 필터는 CSV 내보내기와
같은 것을 쓴다.

**스키마를 기억으로 쓰지 않았다.** class_uid·category_uid·severity_id·status_id·
activity_id·observable type_id·analytic type_id 를 전부 `schema.ocsf.io/api/1.1.0`
에서 확인했다. 그 과정에서 `cloud`·`action_id` 가 필수로 보이지만 실은 각각
cloud·security_control **프로필이 적용될 때만** 필수임을 확인했다 — 온프렘이라
그 프로필을 쓰지 않으므로 선언하지 않는다. 선언하지 않은 프로필의 필드를 채우는
척하는 것이 빈칸으로 두는 것보다 나쁘다.

**표준 준수를 자기 코드로 검증하면 그건 검증이 아니다.** 내가 만든 매핑을 내가
만든 어서션으로 확인하면 둘 다 같은 오해를 공유한다. 독립 라이브러리
`py-ocsf-models` 로 검증했고, **실제로 그것이 `analytic.uid` 누락을 잡아냈다**
(공식 스키마는 권장이지만 라이브러리는 필수로 강제한다 — 어차피 채우는 편이
맞다. 룰 ID 가 없으면 threat_type 이 곧 룰의 정체다).

실측: 실데이터 2,000건 · API 응답 500줄 전부 검증 통과.

**과장하지 않는다**: OCSF 핵심 필드의 **부분집합**이다. 인증받은 완전 매핑이 아니다.
NDJSON 으로 주는 이유는 SIEM 수집기가 기대하는 형태이고, 11만 건을 한 배열로 묶으면
받는 쪽이 통째로 메모리에 올려야 하기 때문이다. 분석가 판정(정탐/오탐)은 표준에
자리가 없지만 이 시스템의 핵심 산출물이라 `unmapped` 확장에 싣는다 — 버리면 안 된다.

테스트 41건.

#### 2. Sigma 룰 CI 검증 (detection-as-code) — ~~강력 추천~~ **구현됨 (2026-08-28)**
...
**구현 (2026-08-28)** — `tests/test_sigma_rules.py` (검사 55건) + CI 의 전용 단계
`탐지 룰 검증 (detection-as-code)`. 룰이 깨졌는지 앱이 깨졌는지 CI 로그에서 바로
갈리도록 전체 테스트보다 먼저 따로 돌린다.

**룰이 자기 테스트를 들고 다닌다.** 각 `.yml` 최상위에 `tests:` 블록을 두고
positive(반드시 매치) / negative(반드시 미매치) 샘플을 적는다. 엔진은 `detection`
만 읽으므로 이 키는 무시된다. 검증은 실제 `SigmaEngine.test_event()` 로 한다 —
별도 매처를 만들면 그건 룰이 아니라 테스트를 검증하는 것이 된다.

**negative 가 이 검증의 핵심이다.** 오탐은 조용히 쌓이다가 분석가가 알림을
무시하게 만드는 방식으로 탐지 체계를 망가뜨린다. 그리고 실제로 하네스를
도입하자마자 오탐 하나를 잡았다:

> `Cryptominer Execution` 룰이 CommandLine 에 `pool` 만 있어도 매치했다.
> 실측으로 `gunicorn --worker-pool=gevent` 와 `java -Dpool.maxSize=20` 같은
> **정상 프로세스를 HIGH 로 올리고 있었다.** 채굴 풀을 특정하는 문자열
> (`stratum+tcp`·`minexmr`·`supportxmr` 등)만 남기고, 그 두 케이스를 negative
> 테스트로 고정했다. 퍼플팀 miner 시나리오는 그대로 탐지된다.

함께 검사하는 것: 필수 필드·UUID 형식의 id·유효한 level/status·id/title 중복·
`attack.tXXXX` 기법 태그와 전술 태그 존재·**기법이 MITRE 매트릭스에 칸을 갖는지**
(제안 A 의 커버리지 진단과 같은 불변식)·**테스트 없는 룰 금지**·퍼플팀 시나리오
5종이 여전히 Sigma 로 탐지되는지.

부수 발견 — `sigma_engine.BUNDLED_RULES`(data/sigma 가 비었을 때 쓰는 사본)가
파일과 어긋나 있었다. 사본이 낡으면 **새 환경이 다른 탐지 로직으로 뜬다.**
파일에서 그대로 생성하도록 바꾸고 동기화 검사를 넣었다. 빈 디렉터리에서
재생성해도 원본과 완전히 동일함을 확인했다(tests 블록 포함).

변이 주입 4종(오탐 문자열 재도입 / 탐지 로직 파손 / 번들 드리프트 / 테스트 없는
룰 추가) 모두 실패 확인.
...
#### 6. Claude 분석 결과의 근거 추적 — ~~추천~~ **C 에 포함되어 구현됨 (2026-08-28)**
...
#### 7. 위협 헌팅 쿼리 콘솔 — ~~조건부 추천~~ **구현됨 (2026-08-29)**
...
**구현 (2026-08-29)** — `modules/hunt.py` + `/api/hunts[/<id>[/run]]` + 전용 패널.
선행 조건(#13 아카이브 조회)이 먼저 끝났으므로 껍데기가 아니다 — 기본
`scope="all"` 로 **아카이브 포함 전체 이력** 위에서 사냥한다(실측 61,457건).

세 가지를 한다:

1. **쿼리를 저장한다.** 분석가의 지식(무엇을 왜 찾는가)이 머릿속이 아니라 실행
   가능한 형태로 남는다. 시작 쿼리 6개를 넣되 **사용자가 지운 것은 되살리지 않는다.**
2. **지난 실행 이후 새로 걸린 것만 센다.** 헌팅에서 궁금한 건 전체 결과가 아니라
   델타다 — 매번 같은 100건을 다시 보여주면 사람은 곧 안 본다. 첫 실행은 전부가
   '새것'이라 델타가 의미 없으므로 `first_run` 으로 그 사실을 숨기지 않고 알린다.
   `mark=0` 미리보기는 기준선을 소진하지 않는다.
3. **찾은 지표를 워치리스트로 승격한다.** 결과가 행동으로 이어지지 않으면 조회일
   뿐이다. 결과에서 반복되는 출발지를 세어 승격 후보로 제시한다.

필터는 `search()` 가 아는 키만 통과시킨다(`ALLOWED_FILTERS`). 저장된 조건이 낡아
`search()` 가 거부해도 콘솔은 오류를 표시할 뿐 죽지 않는다. 허용 목록이 `search()`
시그니처와 어긋나면 테스트가 잡는다 — 어긋나면 조건이 조용히 무시된다.
...
#### A. 탐지 커버리지 자가 진단 (Detection Coverage Gap Report) — ~~최우선 추천~~ **구현됨 (2026-08-28)**
...
**구현 (2026-08-28)** — `modules/coverage.py` + `GET /api/mitre/coverage` + MITRE
패널의 '커버리지' 뷰. 세 축을 조인한다: (A) 룰 존재 = `THREAT_MAPPING` ·
`SYSMON_MAPPING` · Sigma 룰의 `mitre` 태그, (B) 검증 = 퍼플팀 시나리오,
(C) 히트 = `mitre_tracker.hits`. 예상대로 새 수집기는 필요 없었다.

**핵심은 "히트 0"의 해석을 갈라준다는 것이다.** 이전에는 히트 0 인 칸이 '공격이
없었다'인지 '룰이 없어 못 본다'인지 구분되지 않았는데, 이 둘은 정반대의 의미다.
매트릭스에 초록(룰+검증)·노랑(룰만)·빨강 점선(공백)을 덧입혀 그 구분을 만든다.

**만들자마자 실제 결함을 하나 찾았다**: `T1496`(자원 탈취)과 `T1505`(서버
소프트웨어 컴포넌트/웹셸)는 **Sigma 룰과 퍼플팀 시나리오가 둘 다 있는데 매트릭스에
칸이 없어 탐지돼도 표시될 곳이 없었다.** `TECHNIQUES` 에 추가했고, 그 상태로
돌아가지 않도록 `test_shipped_rules_all_have_a_matrix_cell` 이 지킨다.

현재 실측: 기법 41개 중 **룰 보유 23개(56.1%) · 퍼플팀 검증 7개(17.1%) · 공백 18개.**
공백 목록이 그대로 '다음에 써야 할 룰' 목록으로 UI 에 뜬다(제안 #2 Sigma CI 와 직결).

서브기법은 상위로 접는다(`T1059.004` → `T1059`) — 접지 않으면 멀쩡한 커버리지가
공백으로 잘못 보고된다. 비활성 룰은 커버리지로 세지 않는다. 소스가 없거나 예외를
던져도 보고서는 나온다(진단 화면이 진단 대상 때문에 죽으면 안 된다).
테스트 19건, 변이 주입(서브기법 미접힘 / 비활성 룰 포함) 모두 실패 확인.

#### B. 자기 관측성 대시보드 (Self-Telemetry) — ~~추천~~ **구현됨 (2026-08-29)**
...
**구현 (2026-08-29)** — `modules/telemetry.py` + `GET /api/telemetry` + 헬스 패널의
'자기 관측성' 섹션. 지점별 p50/p95/최대 지연, 실패 횟수와 마지막 오류, 큐 적체를
잰다. `/api/system/health` 가 "살아 있는가"를 답한다면 여기는 **"얼마나 느리고 얼마나
실패하는가"** 를 답한다.

**40개 모듈의 `get_health()` 를 고치지 않았다.** `system_health` 와 같은 원칙으로
두 갈래를 썼다 — (1) 문제가 실제로 숨었던 5개 경로에만 `with telemetry.timed()`
를 넣고, (2) 큐 깊이처럼 '지금 값'이 중요한 건 기존 객체에서 방어적으로 꺼내는
프로브로 등록한다(모듈 수정 0). 계측 지점은 감사에서 문제가 났던 곳 그대로다:

| 지점 | 감사 항목 | 숨었던 문제 |
|------|-----------|-------------|
| `incidents.save` | B-1 | 저장 1회 0.63초, 그동안 탐지 경로를 막았다 |
| `ai.analyze` | B-4 | Claude 호출이 최대 30분 스레드를 잡았다 |
| `alert_store.search` | B-7 | 집계가 쓰기를 34초 막았다 |
| `alert_store.write` · `detector.add_alert` | — | 탐지 파이프라인 처리량 |
| `ip_reputation.lookup` | — | 외부 API 지연 |

**느림과 실패를 분리한다.** 대응이 다르기 때문이다 — 한 필드로 뭉뚱그리면 어느
쪽인지 알 수 없다. 임계값은 감사에서 실측한 값을 근거로 지점별로 다르게 둔다.

**구현 중 오탐을 하나 잡았다.** 기동 직후 표본이 4개일 때 콜드스타트 한 건
(DB 초기화 포함)이 p95 를 지배해 `alert_store.write` 가 '느림'으로 표시됐다.
표본 10개 미만이면 판정을 보류하도록 고쳤다(`warming_up`). **경보가 늑대소년이
되면 사람이 화면을 안 본다** — Sigma 룰의 negative 테스트와 같은 이유다.

시계열은 별도 저장소 없이 롤링 윈도우의 최근 40개 표본을 그대로 넘겨 스파크라인
으로 그린다. 계측은 메모리에만 둔다 — 영속화가 필요해지면 그때 붙인다.

테스트 14건. 변이 주입(예외 삼킴 / 느림·실패 뭉개기 / 계측 제거) 모두 실패 확인.

#### C. 차단 결정 재현 로그 (Block Decision Replay) — ~~추천~~ **구현됨 (2026-08-28)**
...
**구현 (2026-08-28)** — `modules/block_decision.py` + `/api/soar/decisions[/<id>[/replay]]`
+ SOAR 패널의 '차단 결정 기록'. 감사 권고대로 #6(AI 근거 추적)을 이 안에 넣었다 —
판정 주체·모델·요약이 신호 스냅샷에 들어가되 **프롬프트 전문은 담지 않는다**
(알림 사본이 생긴다).

**설계 원칙: 기록된 결정이 곧 실제 결정이어야 한다.** 로그를 위해 판정을 다시
계산하면 로그와 행동이 갈라질 수 있고, 그런 로그는 없느니만 못하다. 그래서 게이트
판정을 `evaluate_gates()` 하나로 뽑고 **SOAR 가 그 반환값으로 차단하며** 같은 값을
저장한다.

**차단하지 않은 결정도 남긴다.** 실무에서 더 자주 묻는 질문은 "왜 안 막았나"다.
어떤 게이트가 막았는지가 그 답이고, 그것을 집계하면 무엇이 차단을 가장 자주
막는지가 나온다 — `SOAR_MIN_BLOCK_CONFIDENCE` 튜닝의 출발점이다.

**구현 중 발견 — 게이트 통과와 실제 차단은 다르다.** `_block_ip()` 는 True/False 만
돌려주는데, 그 True 안에 '실제로 막았다'와 '승인 큐에 넣었다'가 섞여 있었다. 게다가
안전장치(사설·Tailscale·화이트리스트·비공인 IP)가 게이트보다 우선한다. 그대로
기록했다면 **로그가 '차단됨'이라 말하는데 실제로는 아닌** 상태가 생겼을 것이다.
`_block_ip(outcome=...)` 로 결과 경로를 받아 `blocked`(실제)와 `gates_passed`(판정)를
분리해 적는다. 결과는 `blocked` / `queued_for_approval` / `prevented_by_safety` /
`already_blocked` / `gates_not_met` 로 갈린다.

재생은 **GET** 이다 — 조회 전용이라 상태변경 엔드포인트의 CSRF 게이트를 붙일 이유가
없다. 원본 레코드를 고쳐 쓰지 않는다(감사 기록이 아니게 된다).

테스트 27건. 변이 주입(게이트 통과를 곧 차단으로 기록)이 안전장치·승인 큐 케이스
양쪽에서 실패로 잡힌다. 보존은 감사 로그와 같은 365일(`BLOCK_DECISION_RETENTION_DAYS`).
