...
**브라우저 순회가 잡는 것은 위 두 계층이 원리적으로 못 본다.** pytest 821건·ruff·GET
스모크가 전부 초록이던 날 이것만 결함 3건을 잡았다 — 인시던트 dict 를 락 밖에서 순회한
스레드 경합 500, favicon 404, 모바일에서 제목 줄 컨트롤 잘림.
...
요약의 `slow` 가 0 이 아닌 지점부터 본다.

실측 기준선 **2026-09-11 재측정**(알림 111,378건·인시던트 23,303건·동시 8, `--with-real-data`):

| 프로파일 | `alert_store.search` p95 | 예산(2초) 초과 |
|---|---|---|
| `legacy` (옛 기준선과 같은 조건) | **444ms** (2026-09-06 의 약 400ms 와 사실상 동일 — 회귀 없음) | 1회차 `/api/metrics/soc?days=90` 2.3초(2회차 캐시로 수 ms) |
| `both` (조사 콘솔 트래픽 포함) | **1,078ms** | `/api/alerts/history/export.ocsf.json?limit=2000` 2.3초 |

읽는 법: 같은 조건에서는 안 느려졌고, **콘솔 질의가 같은 저장소를 함께 때리면
조회 p95 가 두 배가 된다**(새 기능의 값이지 회귀가 아니다). HTTP 실패 0·텔레메트리
`slow` 0. `/api/metrics/soc` 1회차가 1.1초→2.3초로 는 것은 인덱스 문제가 아니라
**메모리에 올린 인시던트 23,303건을 파이썬으로 훑기 때문**이다(캐시 뒤엔 수 ms).
...
※ 룰 `meta.scope` — `auto`(기본)는 자동 스캔에도 쓰고, `manual` 은 수동 스캔
   전용이다. UPX 패킹처럼 **악성이 아니라 정황**인 신호는 시스템 전체를 훑을 때
   소음이지만 분석가가 파일을 지목했을 때는 의미가 있다.
