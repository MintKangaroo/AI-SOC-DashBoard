...
(function () {
    ...
    apply(root) {
      ...
      elements.forEach(el => {
        ...
        if (permission && !this.can(permission)) {
          el.setAttribute('aria-disabled','true'); el.title = 'Requires ' + permission + ' permission. Current role: ' + this.role;
  ...
  for (const type of ['click','change','keydown']) document.addEventListener(type,event => {
    ...
    if (permission && !SOCAccess.can(permission)) {
      ...
      window.SOCUI?.notify('Your ' + SOCAccess.role + ' role does not permit this action.');
    }
  },true);
  document.addEventListener('DOMContentLoaded',() => {
    const badge = document.getElementById('access-role-badge');
    if (badge) badge.textContent = SOCAccess.enforced ? SOCAccess.role.toUpperCase() + ' · server enforced' : 'AUTH OFF · unrestricted';
    const banner = document.getElementById('access-context');
    if (banner) { banner.textContent = SOCAccess.enforced ? 'Role: ' + SOCAccess.role + (SOCAccess.role === 'viewer' ? ' · read-only investigations' : ' · actions require server authorization') : 'AUTHENTICATION OFF · unrestricted local access'; }
