...
- **UI 한국어 통일** — 09-09 에 추가된 조사 큐·계정 권한·탐지 품질·증거 요약·코파일럿 화면의 영어 문구를 기존 한국어 화면과 맞췄습니다. 출처 배지(REAL/DEMO/SIMULATED…)와 API 값은 그대로입니다.
- **상시 운영** — systemd 사용자 서비스(`scripts/soc-dashboard.service`)로 자동 재시작. 문서 잔재(구 인수인계·미사용 로그인 이미지) 제거.
...
- **데이터·안전 설정 유지** — 새 UI 토큰은 기존 팔레트를 공유합니다. 생성 이미지는 로그인에서 사용하지 않으며 09-11 저장소에서 제거했습니다. 탐지·차단·승인·TTL·패치 정책과 데이터베이스는 디자인 복원 대상에 포함하지 않습니다.
...
- **차단 결정 재현** — 자동 차단은 되돌리기 어려운 조치입니다. 결정 시점의 모든 입력 신호(신뢰도·AI 판정·평판·독립 근거)를 고정해 *왜 막았나*와 **왜 안 막았나**를 모두 남기고, 임계값을 바꿨다면 결과가 달라졌을지 재생합니다
...
- **OCSF 내보내기** — 알림 이력을 [OCSF](https://schema.ocsf.io) 1.1.0 Detection Finding(class_uid 2004) NDJSON으로 내보냅니다. 내부 스키마는 그대로 두고 **밖으로 나갈 때만** 표준 형태로 바꿉니다. 표준 준수는 독립 라이브러리(`py-ocsf-models`)로 검증합니다 — 실데이터 2,000건 통과
...
#    상시 운영은 systemd 사용자 서비스로 (WSL 재시작·OOM 뒤 자동 복구):
#    cp scripts/soc-dashboard.service ~/.config/systemd/user/ && systemctl --user enable --now soc-dashboard
