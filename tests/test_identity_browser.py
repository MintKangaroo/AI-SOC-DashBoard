...
def test_viewer_reads_evidence_and_cannot_use_mutation_controls(role_browser):
    ...
    assert drawer.get_by_role('button', name='확인(ACK)', exact=True).is_disabled()
    drawer.get_by_role('button', name='분석 코파일럿', exact=True).click()
    assert drawer.get_by_role('button', name='증거 요약', exact=True).is_disabled()
    ...
    until(page, "document.getElementById('access-status').textContent.includes('관리 모드')")
...
def test_admin_creates_user_and_revokes_existing_socket(role_browser, managed_live):
    ...
    row.get_by_role('button', name='세션 회수').click()
...
def test_analyst_ack_and_self_password_change(role_browser, managed_live):
    ...
    page.evaluate('consoleOpenInvestigation(990001)')
    page.wait_for_selector('#investigation-meta .provenance-demo')
    page.get_by_role('button', name='확인(ACK)', exact=True).click()
    ...
    page.get_by_role('button', name='비밀번호 변경 후 로그아웃').click()
